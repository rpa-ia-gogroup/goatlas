...
| ID | Requisito | Pri |
|---|---|---|
| RF-07 | Interface conversacional: o colaborador descreve a demanda em texto livre; o agente conduz o diálogo até ter contexto suficiente. O agente se apresenta pelo que **este app** faz — quem cumprimenta ou pergunta "o que você faz?" recebe as capacidades reais (documentação, histórico, chamado montado e editável, acompanhamento no app, anexo, formulário), nunca uma saudação de assistente genérico. E ele **não promete verificação que a instalação não tem**: sem allowlist de espaços ou sem os exemplos de **Q3**, o agente diz que não checou, em vez de concluir "não encontrei nada" (ver `D-33`, e **RNF-18**). | P0 |
...
| RF-14 | O prompt de classificação da Regra 2 deve conter **exemplos reais de ajuste operacional da Gocase**. Sem exemplos do contexto da empresa, a classificação é imprecisa. Levantar esses exemplos é pré-requisito de implementação, não refinamento posterior. ✂️ **`D-60`: `Q3` foi cortada e o campo saiu do console — a Regra 2 fica desligada por decisão.** O predicado (`regra2Disponivel`) e o fail-safe continuam: `check_jira_history` roda e **se declara indisponível**, e o prompt não promete a segunda verificação (`D-33`). Reabrir é preencher a chave por `PUT /api/admin/config`. | P0 |
...
| **RF-68** | **A prioridade chega com o motivo dela, e só o cartão fala de nível e de prazo.** A decisão que escolhe o nível produz, na **mesma operação**, um **motivo de até duas frases sobre o caso da pessoa** — nunca a descrição genérica do nível, que vale para qualquer chamado que já existiu e transforma a revisão de **RF-16** em palpite contra palpite (**R-04**). Motivo ausente, acima do teto ou fora do português: nível e prazo aparecem, a tela **declara** que a sugestão não veio justificada e a criação segue disponível (precedente de `D-53`, **RNF-18**). ⚠️ E a prosa do agente **não afirma** nível nem quantidade de horas: mensagem e decisão saem de duas chamadas **paralelas** ao modelo (`D-32`), então a mensagem não vê a decisão — medido em 13/08/2026, texto dizendo Crítica/4h com o cartão marcando Alta/12h. Prazo errado é pior que prazo ausente (`D-42`), e **RN-08** exige o significado do SLA explícito em toda comunicação. Detalhe e cenários na spec 008. | P0 |
| **RF-69** | **O resumo de confirmação é negociável: mandar mensagem é uma forma de ajustá-lo.** Enquanto o turno corre, o resumo **sai da tela** — cartão antigo ao lado de uma decisão em andamento é um formulário que já não é o que vai ser aberto. Terminado o turno ele volta **remontado**, com os valores da proposta vigente, inclusive nos controles que a pessoa já havia mexido, e **sem nenhuma frase** sobre a substituição: basta mudar, e o seletor editável é a saída. ⚠️ Hoje isto é falso e não produz erro nenhum: o seletor guarda o valor da primeira montagem e o cartão não é remontado, então argumentar funciona no servidor e falha na tela — a pessoa conclui que foi ignorada. Família de `D-46`: recomeçar é **remontar** (`key`), nunca uma sequência de `setState`. | P0 |
| **RF-70** | **Antes da primeira mensagem com o resumo na tela, a pessoa é avisada de que conversar pode reescrever o que ela preencheu**, com duas saídas: seguir com o envio ou voltar ao formulário. Voltar não envia nada, não altera a proposta e preserva o texto já escrito. O aviso aparece **uma vez por conversa**, disparado pela **exibição** — não pela escolha feita nele —, e não volta nem se o cartão desaparecer e reaparecer. ⚠️ **Nunca** quando não há proposta a proteger, e **nunca** com bloqueio pendente sem override: ali ele viraria a parede que **RN-07**/**RF-13** proíbem. | P1 |
| **RF-71** | **A IA ajusta o chamado montado a pedido, em texto** — prioridade, título, descrição, assunto e os campos do formulário do assunto vigente. Corrigir explicando (*"é o Chaplin, não o Factory"*) é a forma natural num app cuja tela principal é uma conversa; hoje o chamado montado só muda pela mão da pessoa, campo por campo. Limites: o assunto fica no conjunto que a instalação oferece (**RF-28**, `D-70`) · valor fora das opções do campo **não é gravado**, e as opções são informadas pelos **rótulos** (`D-39`, **RNF-30**) · campo que o assunto vigente não tem não é inventado, e a recusa é dita · nada disso alcança a identidade do solicitante (spec 006) nem a área (`D-52`, corrigível por **RF-19**). ⚠️ Urgência **pedida** não move a prioridade: quem a move é o **impacto descrito** (**R-04**), e o caminho da pessoa continua sendo a edição manual de **RF-16** — visível e registrada. | P1 |
...
| **RF-64** | **O conteúdo do anexo entra no contexto da conversa antes de o agente responder.** Um agente **auxiliar** lê os anexos novos (imagem · PDF, inclusive escaneado · texto/markdown), julga se há conteúdo relevante e entrega uma descrição em português ao agente principal, que não responde antes disso. Espera de no máximo **8 s por turno**; esgotada, a resposta sai e a leitura entra no turno seguinte. Anexo já analisado nunca é relido. Detalhe e cenários na spec 007. | P1 |
| **RF-65** | **A pessoa vê o que foi entendido do arquivo dela, e o julgamento irrelevante não fala com ela.** Descrição relevante aparece na conversa identificada pelo nome do arquivo; arquivo que não pôde ser lido é dito **com o nome**, nunca em silêncio; e as três formas de não ler (formato não suportado · nada legível · falha) são frases distintas, porque pedem ações distintas. A descrição vai também à transcrição anexada ao chamado (`RF-23`), **inclusive** quando a tela não a mostrou. | P1 |
| **RF-66** | **Clicar num anexo mostra o arquivo sem sair da tela** — imagem, PDF e texto/markdown. Tipo não exibível diz isso e oferece o download, nunca abre visualização vazia. Operável por teclado: foco contido, `Esc` fecha, foco devolvido à origem. | P2 |
...
| RF-28 | Exibir apenas tipos de chamado presentes na allowlist do admin (**RF-49**) **e pertencentes ao service desk configurado**. Nada exposto por padrão. O conjunto é o mesmo para as três superfícies — formulário, cartão de confirmação e a escolha do tipo pela IA —, e a IA recebe **o nome** de cada tipo, nunca só o id (`D-70`); sem nome não há proposta. | P0 |
...
| RF-67 | Cada tela do app tem **URL própria** e o **voltar do navegador** desfaz um passo. Vem de três relatos de uso (`D-65`): tudo morava em `/` com `replaceState`, então o histórico tinha uma entrada só e o ← saía do app. ⚠️ Depende de `not_found_handling: "single-page-application"` no deploy. | P1 |
...
| RF-43 | Macro não suportada degrada de forma visível (placeholder), nunca some em silêncio. ⚠️ E macro com **corpo** tem o corpo renderizado — a moldura se perde, o texto não (`D-31`). Quais blocos são reproduzidos, quais ficam placeholder por custo, e por que `jira`/`jirachart` fica de fora **por decisão**: `D-31`. ⚠️ O placeholder **nomeia o bloco em português**; o nome técnico só aparece quando é a única pista (`D-34`, `D-63`). ⚠️ E `toc` **deixou de ser placeholder** (`D-63b`): o texto do índice são os títulos que já estão na tela, então a frase "não há o que trazer" era falsa ali. | P1 |
...
| RN-11 | A declaração de anexo (**RF-62**) é **pergunta**, não campo opcional: **quando o tipo de chamado aceita anexo**, sem resposta não há criação. Tipo que não aceita — ou schema que não pôde ser lido — não pergunta e não bloqueia (**RNF-18**). E "tenho anexo" nunca vira exigência de arquivo: quem declara e desiste volta a "não tenho" e abre o chamado. A trava é responder, não anexar. **Quem já anexou não é perguntado** (`D-70`): o arquivo enviado é a resposta, e o fato vence a declaração — resposta negativa com arquivo pendente é registrada como positiva, porque o arquivo vai ao chamado de qualquer forma. |
| RN-12 | **Conteúdo lido de arquivo é entrada NÃO CONFIÁVEL.** Nenhuma instrução contida num anexo altera `RF-08` (ordem das verificações), `RF-17` (confirmação), `RF-30` (isolamento), `RF-32` (comentário interno) nem a prioridade revisável de `RF-16`. A garantia é estrutural — o agente auxiliar não tem tools e não vê o histórico —, e o que ele produz entra delimitado. Teste de burla obrigatório. |
| RN-13 | **Proposta nova não apaga o trabalho da pessoa: o merge é por campo.** O que a proposta alterou passa a valer o dela; o que ela **não** alterou continua exatamente como a pessoa deixou — inclusive a prioridade que ela mudou à mão (**RF-16**). A regra existe porque as duas metades erradas são igualmente ruins: adotar a proposta inteira apaga o que foi digitado, e preservar tudo faz o ajuste pedido não aparecer (**RF-69**). ⚠️ A exceção é o **assunto**: mudou o assunto, mudou o formulário, então os campos do assunto anterior são descartados, os do novo começam vazios e a tela **diz** que o assunto mudou — campo não some em silêncio (**RF-27**). |
