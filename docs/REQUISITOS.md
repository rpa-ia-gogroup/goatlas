...
| RF-43 | Macro não suportada degrada de forma visível (placeholder), nunca some em silêncio. ⚠️ E macro com **corpo** tem o corpo renderizado — a moldura se perde, o texto não (`D-31`). Quais blocos são reproduzidos, quais ficam placeholder por custo, e por que `jira`/`jirachart` fica de fora **por decisão**: `D-31`. ⚠️ O placeholder **nomeia o bloco em português**; o nome técnico só aparece quando é a única pista (`D-34`, `D-64`). ⚠️ E `toc` **deixou de ser placeholder** (`D-63b`): o texto do índice são os títulos que já estão na tela, então a frase "não há o que trazer" era falsa ali. | P1 |
