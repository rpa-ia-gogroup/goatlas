...
| RF-39 | Renderizar a página com fidelidade razoável (títulos, listas, **listas de tarefas**, tabelas, código, imagens e anexos servidos pelo proxy). ⚠️ O checklist do Confluence entrou em `D-63`: sem ele, cada tarefa chegava como id + `incomplete` + texto, tudo colado. E página **sem conteúdo nenhum** diz que está vazia, em vez de abrir em branco. | P0 |
...
| RF-43 | Macro não suportada degrada de forma visível (placeholder), nunca some em silêncio. ⚠️ E macro com **corpo** tem o corpo renderizado — a moldura se perde, o texto não (`D-31`). Quais blocos são reproduzidos, quais ficam placeholder por custo, e por que `jira`/`jirachart` fica de fora **por decisão**: `D-31`. ⚠️ O placeholder **nomeia o bloco em português**; o nome técnico só aparece quando é a única pista (`D-34`, `D-63`). | P1 |
