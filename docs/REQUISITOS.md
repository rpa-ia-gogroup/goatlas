...
| RN-11 | A declaração de anexo (**RF-62**) é **pergunta**, não campo opcional: **quando o tipo de chamado aceita anexo**, sem resposta não há criação. Tipo que não aceita — ou schema que não pôde ser lido — não pergunta e não bloqueia (**RNF-18**). E "tenho anexo" nunca vira exigência de arquivo: quem declara e desiste volta a "não tenho" e abre o chamado. A trava é responder, não anexar. **Quem já anexou não é perguntado** (`D-68`): o arquivo enviado é a resposta, e o fato vence a declaração — resposta negativa com arquivo pendente é registrada como positiva, porque o arquivo vai ao chamado de qualquer forma. |
