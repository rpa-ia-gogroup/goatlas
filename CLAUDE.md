...
- **O console de admin mostra o que se DECIDE** (`D-15`) — não tudo o que é
  ...
  precisa achar o que importa. Devolvê-los é reabrir `D-15`, não "completar a tela".
...
- ⚠️ **`PUT /api/admin/config` valida o TIPO, não só a chave** (`config/validar.ts`,
  `D-15`). `Config.carregar` faz `JSON.parse` e confia no que está gravado, porque
...
- **O console de admin RELATA o estado, não o recalcula** (`config/diagnostico.ts`,
  `D-15`). Cada predicado é o mesmo que o servidor já aplica — `regra2Disponivel` vem
  ...
  `admin/campos.tsx` é decisão (`D-15`), não esquecimento: TTL, rate limit e teto de
...
**505 testes · typecheck limpo · build limpo**, tudo sem credencial e sem rede.
...
**A aba de admin virou console (spec 003, `D-15`).** Era um scroll único com cinco
...
tela (`D-15`) e `org_id`/`custo_mensal_por_produto` entraram, destravando **Q1**/**Q8**
