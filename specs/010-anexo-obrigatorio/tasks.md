...
- [ ] **T-1000** Rota de diagnóstico só-admin que tenta uma criação e devolve o **corpo do
...
- [ ] **T-1001** Medir na staging: (a) tipo `134` **sem** anexo → ler o corpo do 400 e
...
- [ ] **T-1002** Escrever o desfecho de `M-2` na `spec.md` (§8 deixa de ser
...
- [ ] **T-1005** [teste] `anexos-conteudo.test.ts`: grava 8 MB, lê de volta, confere **SHA-256**;
      fatia de 512 kB; leitura de conversa alheia devolve nada (e-mail no `WHERE`).
      _Requirements: FR-1, ScB-03_
- [ ] **T-1006** `db/schema.ts`: tabela `anexos_conteudo` (`plan.md` §1.1) — **uma linha por
      `INSERT`** (`D-73`). _Requirements: FR-1_
- [ ] **T-1007** `tickets/anexos-conteudo.ts`: repositório com `guardar`, `ler`, `apagar`.
      Base64, fatia de 512 kB, e nenhum método sem e-mail (`RF-30`). _Requirements: FR-1_

## Phase 3 — A trava (servidor primeiro)

- [ ] **T-1008** [teste] `anexo-obrigatorio.test.ts`: `anexoObrigatorio` verdadeiro para schema
...
- [ ] **T-1009** `tickets/declaracao-anexo.ts`: `anexoObrigatorio(schema)`. _Requirements: RN-14_
- [ ] **T-1010** [teste de burla] `ScB-01`: `POST /confirmar` com assunto que exige e zero
...
- [ ] **T-1011** Gate nas **duas** rotas de criação, ao lado de `autorizarDeclaracaoDeAnexo` —
...
- [ ] **T-1030** [teste de burla] `ScB-02`: nenhuma resposta de rota carrega
...
- [ ] **T-1015** [teste] `SC-02`: assunto que exige → o **corpo entregue ao `fetchImpl`** na
...
- [ ] **T-1016** `atlassian/cliente.ts`: `criarChamado` aceita ids temporários, na forma que
      `T-1001` mediu. _Requirements: FR-3_
- [ ] **T-1017** `tickets/anexo-antes-da-criacao.ts` (módulo **novo**, não uma flag em
...
- [ ] **T-1018** [teste] `SC-06`: 5xx no upload da confirmação → submissão **transitória**,
      ...
      SC-06_
- [ ] **T-1019** Apagar os bytes quando a materialização conclui (`plan.md` §1.7), e pegar
      carona no expurgo do outbox para o resto. _Requirements: FR-8, SC-08_

## Phase 6 — A tela

- [ ] **T-1020** [teste] `app/pendencias.ts`: com assunto que exige e zero arquivos, a frase
...
- [ ] **T-1021** Cartão da conversa: aviso de evidência exigida + botão travado + o controle de
...
- [ ] **T-1022** Formulário direto (`Abrir direto`): mesma exigência, mesmo predicado.
