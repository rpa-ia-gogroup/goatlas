...
- [ ] **T-301** Teste de burla do escopo de piloto: e-mail fora da lista não abre
      chamado, e recebe **encaminhamento** (para onde pedir no meio-tempo), não erro
      cru. _Requirements: RNF-30_
- [ ] **T-302** `config.emails_piloto` + gate com tela de encaminhamento. Allowlist
      ...
      over-engineering. **[BLOQUEADA: Q13 para a lista real]**
      _Requirements: R-06_
- [ ] **T-303** [P] `piloto/areas.ts` — mapa `e-mail → área`, função pura. E-mail
      desconhecido → **sem área**, nunca área errada. _Requirements: RF-19_
- [ ] **T-304** Coluna `area` em `vinculos`, preenchida na criação. A área **no
...
- [ ] **T-305** Correção manual da área no recibo, mesmo padrão de `RF-16` com a
...
- [ ] **T-311** [P] `config.baseline_assentos` + comparação antes × depois. Baseline
      ausente **não** inventa número. **[BLOQUEADA: Fase 0, do João]**
      _Requirements: O2_
- [ ] **T-312** [P] Métrica por área e por prioridade, usando `vinculos.area`.
...
- [ ] **Nenhuma `[BLOQUEADA]`** — T-302 (Q13), T-311 (Fase 0), e as 7 `[HUMANO]`
